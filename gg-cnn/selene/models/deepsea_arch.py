...
def get_model(weights_filename=None, updated_model_filename=None):
    ...
    if not (weights_filename is None):
        ...
        new_dict = deepsea_cpu.state_dict()
        for weights, (key, _) in zip(state_dict.values(), new_dict.items()):
            new_dict[key] = weights
        deepsea_cpu.load_state_dict(new_dict)
    return deepsea_cpu
...
class DeepSEA(nn.Module):
    ...
    def __init__(self, weights_filename=None, save_update_to_file=None):
        super(DeepSEA, self).__init__()
        self.model = get_model(weights_filename=weights_filename)
        if not (save_update_to_file is None):
            save_seqpred_model_weights(fname=save_update_to_file)
